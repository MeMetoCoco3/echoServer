...
func main() {
	...
	id1, u1, err := NewUser("Vidal", "Maquine", 12)
	id2, u2, err := NewUser("Katerina", "Principesa", 23)
	id3, u3, err := NewUser("Mare", "Reina", 34)
	id4, u4, err := NewUser("Pare", "The fucking king", 12)
	id5, u5, err := NewUser("Hermano", "Maquine", 12)
	id6, u6, err := NewUser("El abuelo Vidal", "Legend of the game", 12)
	log.Println(id1)
	log.Println(id2)
	log.Println(id3)
	log.Println(id4)
